<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="com.servlet.CheckCustomerServlet,com.entity.Film,java.util.* "%>
...
<script type="text/javascript">
	function check(form) {
		...
		if (document.forms.insertForm.language.value == "") {
			alert("请选择电影语言！");
			document.forms.insertForm.name.focus();
			return false;
		}
	}
</script>
...
<body>
	<form action="<%=request.getContextPath()%>/InsertServlet"
		...
		<table border="1" cellspacing="0" cellpadding="6" bordercolor="silver"
			align="center">
			<tr>
			...
			<tr>
				<td>
				电影语言：<select style="width=350px;height=25px;">		    			
				<option value="${laguage.languageId}">English</option>
				<option value="${laguage.languageId}">French</option>
				<option value="${laguage.languageId}">German</option>
				<option value="${laguage.languageId}">Italian</option>
				<option value="${laguage.languageId}">Japanese</option>
				<option value="${laguage.languageId}">Mandarin</option>
			</td>
			</select>	
